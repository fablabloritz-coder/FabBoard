...
(function initMissions() {
    const modalEl = document.getElementById('modalMission');
    if (!modalEl) return;
    missionModal = new bootstrap.Modal(modalEl);

    document.getElementById('btn-add-mission').addEventListener('click', openCreateMissionModal);
    document.getElementById('btn-save-mission').addEventListener('click', saveMission);
    document.getElementById('missions-table-body').addEventListener('click', onMissionsTableClick);
...
function renderMissionsTable() {
    ...
    if (missions.length === 0) {
        tbody.innerHTML = '<tr><td colspan="4" class="text-center text-muted py-3">Aucune mission</td></tr>';
        return;
    }

    const statutLabels = { a_faire: 'À faire', en_cours: 'En cours', termine: 'Terminé' };
    const statutClasses = { a_faire: 'bg-secondary', en_cours: 'bg-warning text-dark', termine: 'bg-success' };
    const prioLabels = { 0: 'Normale', 1: 'Haute', 2: 'Urgente' };
    const prioClasses = { 0: 'text-muted', 1: 'text-warning', 2: 'text-danger fw-bold' };

    tbody.innerHTML = missions.map(m => {
        return '<tr>' +
            '<td>' + escapeHtml(m.titre) + (m.description ? '<br><small class="text-muted">' + escapeHtml(m.description) + '</small>' : '') + '</td>' +
            '<td><span class="badge ' + (statutClasses[m.statut] || '') + '">' + (statutLabels[m.statut] || m.statut) + '</span></td>' +
            '<td><span class="' + (prioClasses[m.priorite] || '') + '">' + (prioLabels[m.priorite] || 'Normale') + '</span></td>' +
            '<td class="text-end">' +
            '<button class="btn btn-sm btn-outline-primary me-1" data-action="edit-mission" data-id="' + m.id + '" title="Modifier"><i class="bi bi-pencil"></i></button>' +
            '<button class="btn btn-sm btn-outline-danger" data-action="delete-mission" data-id="' + m.id + '" title="Supprimer"><i class="bi bi-trash"></i></button>' +
            '</td></tr>';
    }).join('');
}

function onMissionsTableClick(event) {
    const btn = event.target.closest('[data-action]');
    if (!btn) return;
    const action = btn.dataset.action;
    const id = parseInt(btn.dataset.id);
    if (action === 'edit-mission') openEditMissionModal(id);
...
function openCreateMissionModal() {
    ...
    document.getElementById('mission-titre').value = '';
...
function openEditMissionModal(id) {
    ...
    document.getElementById('mission-titre').value = m.titre;
...
async function saveMission() {
    ...
    const titre = document.getElementById('mission-titre').value.trim();
    ...
    try {
        if (id) {
            await apiCall('/api/missions/' + id, 'PUT', payload);
            showToast('Mission modifiée', 'success');
        } else {
            await apiCall('/api/missions', 'POST', payload);
            showToast('Mission créée', 'success');
        }
        missionModal.hide();
        await loadMissions();
    } catch (e) {
        showToast('Erreur : ' + (e.message || 'Erreur'), 'error');
    }
}
...
async function deleteMission(id) {
    ...
    try {
        await apiCall('/api/missions/' + id, 'DELETE');
